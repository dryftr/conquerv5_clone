...
/* Modern in-memory lock structure to replace file-based locking */
typedef struct {
    int is_locked;
    char locked_by[32];
    time_t lock_time;
...
static MEMORY_LOCK memory_locks[3] = {
    {0, "", 0, 0},
    {0, "", 0, 0},
    {0, "", 0, 0}
};

/* MEMORY_LOCK_ACQUIRE -- Modern in-memory lock acquisition */
static int
memory_lock_acquire PARM_2 (int, slot, char *, nation)
{
  if (slot < 0 || slot > 2) {
    return -1;
  }
  
  /* If already locked by this nation, allow re-entry */
  if (memory_locks[slot].is_locked && 
      strcmp(memory_locks[slot].locked_by, nation) == 0) {
    memory_locks[slot].ref_count++;
    return 0;
  }
  
  /* If locked by another nation, fail */
  if (memory_locks[slot].is_locked) {
    return -1;
  }
  
  /* Acquire the lock */
  memory_locks[slot].is_locked = 1;
  strncpy(memory_locks[slot].locked_by, nation, 31);
  memory_locks[slot].locked_by[31] = '\0';
  memory_locks[slot].lock_time = time(NULL);
  memory_locks[slot].ref_count = 1;
  
  return 0;
}

/* MEMORY_LOCK_RELEASE -- Modern in-memory lock release */
static void
memory_lock_release PARM_2 (int, slot, char *, nation)
{
  if (slot < 0 || slot > 2) {
    return;
  }
  
  /* Only release if locked by this nation */
  if (memory_locks[slot].is_locked && 
      strcmp(memory_locks[slot].locked_by, nation) == 0) {
    memory_locks[slot].ref_count--;
    
    /* Actually release when ref count reaches 0 */
    if (memory_locks[slot].ref_count <= 0) {
      memory_locks[slot].is_locked = 0;
      memory_locks[slot].locked_by[0] = '\0';
      memory_locks[slot].lock_time = 0;
      memory_locks[slot].ref_count = 0;
    }
  }
}

/* MEMORY_LOCK_CHECK -- Check if slot is locked */
static int
memory_lock_check PARM_2 (int, slot, char *, nation)
{
  if (slot < 0 || slot > 2) {
    return -1;
  }
  
  /* If locked by this nation, allow access */
  if (memory_locks[slot].is_locked && 
      strcmp(memory_locks[slot].locked_by, nation) == 0) {
    return 0;
  }
  
  /* If locked by another nation, deny access */
  if (memory_locks[slot].is_locked) {
    return -1;
  }
